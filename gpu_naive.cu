...
void encryptdemo(uint8_t key[32], uint8_t *buf, unsigned long numbytes){
  ...
  checkCuda(cudaMemcpyToSymbol(sbox, sbox, sizeof(uint8_t)*256));
 ...
 // checkCuda(cudaMallocHost((void**)&ctx_enckey_d, sizeof(ctx_enckey)));
  ...
  checkCuda(cudaEventRecord( start,0)); // start record
  checkCuda(cudaMemcpy(buf_d, buf, numbytes, cudaMemcpyHostToDevice)); // 1. copy buf from host to device
  checkCuda(cudaEventRecord( stop,0)); // end record
  checkCuda(cudaEventSynchronize(start)); // waits for an event to complete
  checkCuda(cudaEventSynchronize(stop));    //Waits for an event to complete
  checkCuda(cudaEventElapsedTime(&time_h_to_d_buf,start,stop)); // calculate delta time
  ...
  checkCuda(cudaEventRecord( start,0)); // start record
  checkCuda(cudaMemcpy(ctx_enckey_d, ctx_enckey, sizeof(ctx_enckey), cudaMemcpyHostToDevice)); // 2. copy key from host to device
  checkCuda(cudaMemcpy(ctx_key_d, ctx_key, sizeof(ctx_key), cudaMemcpyHostToDevice));
  checkCuda(cudaEventRecord( stop,0)); // end record
  checkCuda(cudaEventSynchronize(start)); // waits for an event to complete
  checkCuda(cudaEventSynchronize(stop));    //Waits for an event to complete
  checkCuda(cudaEventElapsedTime(&time_h_to_d_key,start,stop)); // calculate delta time
...
aes256_encrypt_ecb<<<dimBlock, dimGrid>>>(buf_d, numbytes, ctx_enckey_d, ctx_key_d); // 3. kernel
  ...
  checkCuda(cudaEventSynchronize(start)); // waits for an event to complete
  checkCuda(cudaEventSynchronize(stop));    //Waits for an event to complete
  checkCuda(cudaEventElapsedTime(&time_k,start,stop)); // calculate delta time
 ...
 checkCuda(cudaEventRecord( start,0)); // start record
  checkCuda(cudaMemcpy(buf, buf_d, numbytes, cudaMemcpyDeviceToHost)); // 4. copy buf from device to host
  checkCuda(cudaEventRecord( stop,0)); // end record
 checkCuda(cudaEventSynchronize(start)); // waits for an event to complete
  checkCuda(cudaEventSynchronize(stop));    //Waits for an event to complete
  checkCuda(cudaEventElapsedTime(&time_d_to_h_buf,start,stop)); // calculate delta time


  // Record device to host key
  checkCuda(cudaEventRecord( start,0)); // start record
  checkCuda(cudaMemcpy(ctx_enckey, ctx_enckey_d, sizeof(ctx_enckey), cudaMemcpyDeviceToHost)); // 5. copy key from device to host
  checkCuda(cudaMemcpy(ctx_key, ctx_key_d, sizeof(ctx_key), cudaMemcpyDeviceToHost));
  checkCuda(cudaEventRecord( stop,0)); // end record
  checkCuda(cudaEventSynchronize(start)); // waits for an event to complete
  checkCuda(cudaEventSynchronize(stop));    //Waits for an event to complete
  checkCuda(cudaEventElapsedTime(&time_d_to_h_key,start,stop)); // calculate delta time
...
void decryptdemo(uint8_t key[32], uint8_t *buf, unsigned long numbytes){
  ...
  checkCuda(cudaMemcpyToSymbol(sboxinv, sboxinv, sizeof(uint8_t)*256));
...
checkCuda(cudaMemcpy(buf_d, buf, numbytes, cudaMemcpyHostToDevice)); // 1/ copy buf from host to device
  ...
  checkCuda(cudaEventRecord( start,0)); // start record
  checkCuda(cudaMemcpy(ctx_deckey_d, ctx_deckey, sizeof(ctx_deckey), cudaMemcpyHostToDevice)); // 2. copy key from host to device
  checkCuda(cudaMemcpy(ctx_key_d, ctx_key, sizeof(ctx_key), cudaMemcpyHostToDevice));
  checkCuda(cudaEventRecord( stop,0)); // end record
  checkCuda(cudaEventSynchronize(start)); // waits for an event to complete
  checkCuda(cudaEventSynchronize(stop));    //Waits for an event to complete
...
aes256_decrypt_ecb<<<dimBlock, dimGrid>>>(buf_d, numbytes, ctx_deckey_d, ctx_key_d);  // 3. kernel
  ...
  checkCuda(cudaEventRecord( start,0)); // start record
  checkCuda(cudaMemcpy(ctx_deckey, ctx_deckey_d, sizeof(ctx_deckey), cudaMemcpyDeviceToHost)); // 5. copy key from device to host
  checkCuda(cudaMemcpy(ctx_key, ctx_key_d, sizeof(ctx_key), cudaMemcpyDeviceToHost));
  checkCuda(cudaEventRecord( stop,0)); // end record
  checkCuda(cudaEventSynchronize(start)); // waits for an event to complete
  checkCuda(cudaEventSynchronize(stop));    //Waits for an event to complete
...
int main(int argc, char** argv){
  ...
  if (argc < 2)
  {
	  printf("Give a valid text file to run AES on");
	  exit(EXIT_FAILURE);
  }
  // open file
  FILE *file;
