/* Author Nishant Pani
 This CUDA code is modified from the Original CPU code written Canhui Wang
 The modificatios and parallelizations are based on two papers
 1.GPU Accelerated AES Algorithmi, Canhui Wang Xiaowen Chu
 2. Optimization of Advanced Encryption Standard on Graphics Processing Units,Cihangir Tezcan
...
int main(int argc, char* argv[]){
     ...
     if(argc !=2)
     {
       printf("Enter the input txt file");
     }

// 1. Modify to take input arguments
//
  // open file
  FILE *file;
  uint8_t *buf;
  ...
  // handle txt file
  //Understand how the file handling works
  //
  fname = argv[1];
  file = fopen(fname, "r");
  ...
  // Change this to use rand function
